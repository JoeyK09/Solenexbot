...
async function handleUpdate(botId, secretTokenFromHeader, update) {
  ...
  try {
    await trackContact(bot.id, update);
...
/**
 * Upserts the sender of an inbound message as a known contact for this
 * bot, so broadcasts have someone to send to regardless of bot template.
 */
async function trackContact(botId, update) {
  const from = update.message?.from;
  if (!from || from.is_bot) return;
  await db.query(
    `INSERT INTO bot_contacts (bot_id, telegram_user_id, telegram_username)
     VALUES ($1, $2, $3)
     ON CONFLICT (bot_id, telegram_user_id)
     DO UPDATE SET last_seen_at = NOW(), telegram_username = EXCLUDED.telegram_username`,
    [botId, from.id, from.username || null]
  );
}
