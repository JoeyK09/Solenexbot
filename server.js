...
app.post('/store-pay/mpesa/:storeId/:productId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(renderResultPage({ heading: 'Payment failed', message: "Couldn't start the M-Pesa payment. Please try again.", theme: store.config?.theme }));
  }
});
...
app.post('/store-pay/paypal/:storeId/:productId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(renderResultPage({ heading: 'Payment failed', message: "Couldn't start the PayPal payment. Please try again.", theme: store.config?.theme }));
  }
});
...
app.get('/store-pay/paypal/return/:storeId/:productId', async (req, res) => {
  ...
  try {
    ...
    return res.send(renderResultPage({ heading: 'Payment could not be completed', message: 'Please go back and try again.', theme: store.config?.theme }));
  } catch (err) {
    console.error('Store PayPal capture error:', err);
    res.status(500).send(renderResultPage({ heading: 'Something went wrong', message: 'Please contact the seller.', theme: store.config?.theme }));
  }
});
...
app.post('/store-pay/crypto/:storeId/:productId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).send(renderResultPage({ heading: 'Payment failed', message: "Couldn't start the crypto payment. Please try again.", theme: store.config?.theme }));
  }
});
...
app.get('/store-pay/status/:paymentId', async (req, res) => {
  ...
  const storeForTheme = payment.meta_json?.storeId ? await storeManager.getStoreById(payment.meta_json.storeId) : null;
  ...
  if (payment.status === 'completed') {
    ...
    return res.send(renderResultPage({
      ...
      deliveryInfo: product?.delivery_info,
      receiptUrl: `${process.env.BASE_URL}/store-pay/status/${payment.id}`,
      theme: storeForTheme?.config?.theme,
    }));
  }
  if (payment.status === 'failed') {
    return res.send(renderResultPage({ heading: 'Payment failed', message: 'Please go back and try again.', theme: storeForTheme?.config?.theme }));
  }
  return res.send(renderResultPage({ heading: 'Still waiting', message: 'Payment not confirmed yet -- refresh this page in a moment.', theme: storeForTheme?.config?.theme }));
});
