  ...
  state.bots.forEach(bot => {
    ...
    card.innerHTML = `
      ...
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:${bot.template_type === 'subscription' ? 'var(--magenta)' : bot.template_type === 'ai' ? 'var(--violet)' : 'var(--blue)'};">${bot.template_type === 'subscription' ? 'Subscription' : bot.template_type === 'ai' ? 'AI Assistant' : 'FAQ'}</span>
...
function renderSites() {
  ...
  state.sites.forEach(site => {
    ...
    card.innerHTML = `
      ...
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:${site.site_type === 'landing' ? 'var(--violet)' : 'var(--sky)'};">${site.site_type === 'landing' ? 'Landing' : 'Link-in-bio'}</span>
...
function renderStores() {
  ...
  state.stores.forEach(store => {
    ...
    const themeLabels = { mono: 'Minimal Mono', vivid: 'Vivid Gradient', boutique: 'Warm Boutique', neon: 'Bold Neon' };
    const themeLabel = themeLabels[store.config?.theme] || themeLabels.mono;
    card.innerHTML = `
      <div class="bot-card-head">
        <span class="pulse-dot"></span>
        <span class="bot-name">${escapeHtml(store.config?.title || store.slug)}</span>
      </div>
      <p class="bot-username">${escapeHtml(url)} · ${escapeHtml(themeLabel)}</p>
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:var(--magenta);">Store</span>
...
storeForm.addEventListener('submit', async (e) => {
  ...
  const config = { title, description, theme, paymentMethods: [] };
...
function renderBookingPages() {
  ...
  state.bookingPages.forEach(page => {
    ...
    card.innerHTML = `
      ...
      <p class="bot-username">${escapeHtml(url)}</p>
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:var(--teal);">Booking</span>
...
function renderEmailLists() {
  ...
  state_emailLists.forEach(list => {
    ...
    card.innerHTML = `
      ...
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:var(--sky);">List</span>
...
function renderWhatsappChannels() {
  ...
  state_whatsappChannels.forEach(ch => {
    ...
    card.innerHTML = `
      ...
      <div class="bot-meta-row">
        <span class="bot-type-tag" style="color:var(--wa-green);">WhatsApp</span>
  ...
  try {
    ...
    result.listings.forEach(l => {
      ...
      card.innerHTML = `
        ...
        <div class="bot-meta-row">
          <span class="bot-type-tag" style="color:var(--amber);">${escapeHtml(l.category)} · ${escapeHtml(priceLabel)}</span>
